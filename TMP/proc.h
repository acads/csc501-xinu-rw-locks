...
struct	pentry	{
    ...
    /* PA2 stuff */
    int     plid;                       /* waiting on this lock */
    int     plid_type;                  /* waiting on this type of lock */
    int     oprio;                      /* holds the original prio during PI */
    int     prioflag;                   /* inherited or original priority? */
    int     nlocks;                     /* number of locks owned */
    unsigned long plocktime;            /* time when this proc is put in queue */
    unsigned char plidmap[7];           /* lock bit vector */
};
